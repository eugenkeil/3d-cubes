...
plotMain: function(data,color){
	...
	for(n=0;n<data.length;n++){
		...
		if(pd.length==1){ // draw points
			...
			ctx.fill();
		} else if(pd.length==2) { // draw lines
			ctx.lineWidth=2;
			...
			ctx.closePath();
			ctx.stroke();
		} else if(pd.length > 2) { // draw areas, volumes, etc
			ctx.lineWidth=1;
